			...
			<% 
			   ...
			   if(s.getAttribute("pid")!=null){
				   if(((String)s.getAttribute("pid")).equals("admin")){%>
					   <p class="text-right"><a href="<%=request.getContextPath()%>/write_view.service" class="btn btn btn-warning">관리자글쓰기</a></p><%} }%>
				 	   <p class="text-right"><a href="<%=request.getContextPath()%>/write_view.consult" class="btn btn-danger">1:1문의하기</a></p> 
		</div>
</div>
